...
module.exports = {
    ...
    execute(message, args) {
        ...
        let { mute } = require(`../server/${message.guild.id}/CommandVerif.json`);
        if (mute === false) return message.reply(config.MsgDisable);
        ...
        user.roles.add(setup.Muted).then((u) => {
            let msg = channelToSend.send(`L'utilisateur **${user.displayName}** à été mute par **${message.author.username}** pour la raison suivante : **${reason}**`);
            msg.then((m) => {
                ...
                channelForSend.send(`<@` + user.id +`> à été mute du server !`);
